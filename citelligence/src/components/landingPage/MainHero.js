...
const MainHero = () => {
  ...
  return (
    <main className="flex flex-col items-center justify-center w-full gap-10 mt-20">
      ...
      <p className='text-xl w-[550px] font-medium text-center'>
        A plateform to Discover relevent Schemes,
        and to find best relevent among them
      </p>
      <div className='flex gap-5 font-medium'>
        <button className='bg-primary text-white text-lg w-56 p-2 rounded-xl hover:-translate-y-1 duration-150 hover:shadow-lg'>
          Explore Schemes <FontAwesomeIcon icon={faWpexplorer} />
        </button>
        <button className='border border-primary text-primary text-lg w-56 p-2 rounded-xl hover:-translate-y-1 duration-150 hover:shadow-lg'>
          <FontAwesomeIcon icon={faComments} /> Have question?
        </button>
      </div>
      <div>
        <div className="absolute z-20 flex h-[32rem] w-[52rem] backdrop-blur-[1px] rounded-xl justify-center text-lg font-medium items-center text-white group" style={play ? { backdropFilter: 'blur(0)' } : { backdropFilter: 'blur(1px)' }}>
          {!play
            ?
        ...
        <div className="z-10 flex h-[32rem] bg-slate-100 w-[52rem] rounded-xl cursor-pointer justify-center items-center">
